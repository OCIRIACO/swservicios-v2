...
@Component({
  selector: 'app-detalle-autotransporte',
  templateUrl: './detalle-autotransporte.component.html',
  styleUrls: ['./detalle-autotransporte.component.css']
})
export class DetalleAutotransporteComponent implements OnInit {
  ...
  lbltruta:string=''
  lblttramite:string=''
  lblttipocarga:string='';
  lblttipotransporte:string=''
  ...
  ngOnInit(): void {
    ...
    this.e_consultaSolicitudServicio(this.id);

  }

  e_consultaSolicitudServicio(datos: any) {
    let Iparametros: any;
  ...
  e_procesarDatosPost(datos: any) {
    ...
    } else if (datos.testatus == 'CANCELADA') {
      this.lblalert = "SOLICITUD CANCELADA"
      this.lbltipoAlerta = "danger"
      this.lblusuarioInterno = 'Cancelada'
      this.lblFechaInterno = 'Fecha cancelada'
    ...
    this.lbltruta           = datos['operaciones'][0]['truta'];
    this.lblttramite        = datos['operaciones'][0]['ttramite'];
    this.lblttipocarga      = datos['operaciones'][0]['ttipocarga'];
    this.lblttipotransporte = datos['operaciones'][0]['ttipotransporte'];
  ...
  e_imprimir() {
    ...
    html2canvas(data, options).then((canvas) => {
      ...
      doc.text('+ Operacion', 10, xTitle)
      doc.setFont(tfontname, tfontstylebold)
      doc.text('Ruta', 15, xHeader)
      doc.setFont(tfontname, "normal")
      doc.text(this.lbltruta, 15, xValor)

      doc.setFont(tfontname, tfontstylebold)
      doc.text('Trámite', 60, xHeader)
      doc.setFont(tfontname, "normal")
      doc.text(this.lblttramite, 60, xValor)

      doc.setFont(tfontname, tfontstylebold)
      doc.text('Carga', 120, xHeader)
      doc.setFont(tfontname, "normal")
      doc.text(this.lblttipocarga, 120, xValor)

      doc.setFont(tfontname, tfontstylebold)
      doc.text('Tipo transporte', 165, xHeader)
      doc.setFont(tfontname, "normal")
      doc.text(this.lblttipotransporte, 165, xValor)



      /*------------------SERVICIOS-----------------------*/
      var headers = [
        { header: 'Tipo mercancia', dataKey: 'ttipomercancia' },
        { header: 'Tipo servicio', dataKey: 'ttiposervicio' },
        { header: 'Servicio', dataKey: 'ttiposolicitud' }
      ]
      ...
      datosPdfServcios.forEach((dato: any, index: any) => {
        ...
        datoServicio.ttipomercancia = dato.ttipomercancia
        datoServicio.ttiposervicio = dato.ttiposervicio
        datoServicio.ttiposolicitud = dato.ttiposolicitud
      ...
      datoPdf['operaciones'].forEach((operaciones: any, index: any) => {

        operaciones.bienes.forEach((bien: any, index: any) => {
