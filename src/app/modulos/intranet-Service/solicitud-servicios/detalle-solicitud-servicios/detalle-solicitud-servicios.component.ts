...
@Component({
  selector: 'app-detalle-solicitud-servicios',
  templateUrl: './detalle-solicitud-servicios.component.html',
  styleUrls: ['./detalle-solicitud-servicios.component.css']
})
export class DetalleSolicitudServiciosComponent implements OnInit {
  ...
  //Cosnultar detalles
  e_consulta() {
    this.router.navigate(['dashboard/intranet/servicios/solicitudes']);
  }
  ...
  e_procesarDatosPost(datos: any) {
    ...
    //Concat notificaciones 
    this.lblNotificacionRechazo = ''
  ...
  e_autorizar() {
    ...
    parametros = {
      etransaccion: this.route.snapshot.params['id'],  // Folio web de GET  url
      testatus: 'AUTORIZADA',
      tobservaciones: ''
    }
    ...
    this.alertaConfirm(alerta, (confirmed: boolean) => {
      if (confirmed == true) {
        this.apiSolicitudServicios.postActualizarEstadoSolicitudServicio(parametros).subscribe(
          (response) => {
